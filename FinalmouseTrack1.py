...
def chooseOption(tc, ic, mc): 
    ...
    if ic[0]!=-1 and mc[0]!=-1:
        if (distance(tc,ic) < 35) and (distance(tc,mc) > 35):
            prevSc = 0
            return 'left'
        elif (distance(tc,mc) < 35) and (distance(tc,ic) > 35):
            prevSc = 0
            return 'right'
        elif (distance(ic,mc) < 35) and (distance(tc,ic) > 100) and (distance(tc, mc) > 40):
            return 'scroll'
        elif (distance(tc,ic) < 40) and (distance(tc,mc) < 40) and (distance(ic, mc) < 40):
            prevSc = 0
            return 'drag'
        else:
            prevSc = 0
